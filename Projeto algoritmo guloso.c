...
celula_mochila ** mochila(int * S, int n, int K) {
  ...
  celula_mochila ** R = (celula_mochila ** ) malloc((n + 1) * sizeof(celula_mochila * ));
  for (int a = 0; a <= n; a++) {
    R[a] = (celula_mochila * ) malloc((K + 1) * sizeof(celula_mochila));
  }
  R[0][0].exists = true;
  R[0][0].belongs = true;
  for (int r = 1; r <= K; r++) {
...
int main() {
  ...
  int r[] = {
    ...
    5,
    6
  }; // itens a serem possivelmente salvos na celula_mochila
  int K = 9;
  n = 4;
  int itensmochila[] = {
    -1,
